  ...
  >
    <ul>
      ...
      >
        ...
        <ul>
          <li
            v-for="(card,index) in cardgroup.items"
            :key="index"
            class="card"
            @click="select(card,colorList[index%7])"
          >
            <div
              :style="{backgroundColor:'#'+colorList[index%7]}"
              class="avatar"
            >{{ userNameCharAt(card) }}</div>
            <div class="more" />
